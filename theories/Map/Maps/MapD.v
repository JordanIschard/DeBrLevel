...
Lemma valid_Add_iff : forall lb x v m m',
...
Proof.
  ...
  - rewrite <- valid_Add_iff in H2; eauto.
    rewrite Add_eq_spec in H0; eauto. rewrite <- valid_Add_iff; eauto.
  
  - now apply valid_Empty_spec.
  - rewrite <- valid_Add_iff; eauto.
    rewrite Add_eq_spec in H0; eauto. rewrite <- valid_Add_iff in H2; eauto.
Qed.
...
Proof.
  ...
  - rewrite <- valid_Add_iff in Hvm; eauto; destruct Hvm as [Hvx Hvm].
...
Proof.
  ...
  induction m using map_induction; intros lb y v.
  - intros []. rewrite valid_add_notin_spec; try now split.
    intro i; unfold Empty in H; destruct i; exfalso. 
    now apply (H y x).
  - intros []. rewrite <- valid_Add_iff in H2; eauto; destruct H2.
...
Lemma valid_Add_spec : forall lb x v m m',
  ...
  Data.valid lb v /\ valid lb m -> valid lb m'.
Proof.
  intros. unfold Add in H. rewrite H. apply valid_add_spec; assumption.
Qed.
...
Proof.
  ...
  - apply (valid_Add_iff lb x0 e m1 m2) in H1 as [Hvr Hvm1]; auto.
...
Lemma shift_find_iff : forall lb k key v (m : t), 
...
Proof.
  ...
  - eapply fold_Empty with (i := M.empty) (eqA := eq) in H; eauto.
    rewrite Empty_eq_spec; eauto; apply empty_1.
  - unfold Empty in *; intros; intro.
    apply (H x (Data.shift lb k e)).
    apply find_2.
    apply find_1 in H0.
    now apply shift_find_iff. 
Qed.
...
Proof.
  ...
  - intro y. destruct (In_dec m y).
    -- destruct i; apply find_1 in H0. rewrite H0; symmetry.
        erewrite shift_find_iff in H0; rewrite H in H0. 
        now rewrite <- shift_find_iff in H0.
    -- apply not_in_find in n as n''; rewrite n''; clear n''. 
       rewrite shift_notin_iff in n.
       apply not_in_find in n as n'. rewrite H in n'.
       symmetry; destruct (In_dec m1 y).
       + destruct i. apply find_1 in H0.
         rewrite shift_find_iff in H0; rewrite n' in H0; inversion H0.
       + now apply not_in_find.
Qed.

Lemma shift_Add_iff : forall lb k key v (m m' : t),
  Add key v m m' <->
  Add key (Data.shift lb k v) (shift lb k m) (shift lb k m').
Proof.
  intros; split.
  - apply shift_Add_spec.
  - intros; unfold Add in *.
    rewrite <- shift_add_spec in H. 
...
Proof.
  ...
  - apply shift_Add_spec with (lb := lb) (k := 0) in H0 as H0'; auto.
    ...
    -- repeat rewrite add_eq_o; auto; try now symmetry. f_equal.
       apply Data.eq_leibniz. rewrite Data.shift_zero_refl; try reflexivity.
...
- apply shift_Add_iff with (lb := lb) (k := k) in H0 as H1; auto.
  apply shift_Add_iff with (lb := lb) (k := k') in H0 as H1'; auto.
  apply shift_Add_spec with (lb := lb) (k := k') in H1;
  apply shift_Add_spec with (lb := lb) (k := k) in H1';
  try (now rewrite <- shift_notin_iff).
  rewrite H1,H1'.
...
- rewrite shift_Add_spec; eauto. symmetry.
  eapply shift_Add_iff with (lb := lb) (k := k) in H0 as H0'; auto.
...
- repeat rewrite shift_Empty_spec; auto; try reflexivity. 
- eapply shift_Add_iff with (lb := k) (k := (k' - k)) in H0 as H0'; auto.
...
- assert (~ M.In x (shift lb k t0)).
  { 
    apply shift_notin_iff; intro; unfold Empty in *.
...
- rewrite shift_Empty_spec in Heq; auto.
  ...
  rewrite shift_find_iff with (lb := lb) (k := k) in Hfi.
...
- unfold Add in H0; rewrite H0 in *. 
  ...
  { erewrite shift_find_iff; rewrite <- Heq. now apply add_eq_o. }
  ...
  { 
    ...
    -- apply Key.eq_leibniz in e0; subst; 
      rewrite shift_notin_iff in H. apply not_in_find in H; rewrite H.
      rewrite shift_notin_iff in H1. apply not_in_find in H1; now rewrite H1.
...
- now apply valid_Empty_spec.
- rewrite <- valid_Add_iff with (lb := k) in Hvt; eauto.
  rewrite <- valid_Add_iff with (lb := k'); eauto.
...
- eapply valid_Add_iff in Hvt; eauto; destruct Hvt as [Hv Hvt'].
  apply IHt1 with (lb := lb) (k := k) (k' := k') in Hvt'.
  apply Data.shift_preserves_valid_1 with (lb := lb) (k' := k') in Hv.

  apply shift_Add_iff with (lb := lb) (k := k') in H0; auto.
  rewrite <- valid_Add_iff; eauto.
...
- eapply valid_Add_iff in H6; eauto; destruct H6 as [HvData Hvt].
  eapply IHt1 in Hvt; eauto.
  apply shift_Add_iff with (lb := k) (k := k' - k) in H0; auto.
  rewrite <- valid_Add_iff with (m := (shift k (k' - k) t1)); eauto.
  -- split; auto.
