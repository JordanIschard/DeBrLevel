...
Lemma valid_add_iff : forall m x (v : Data.t) lb,
...
Lemma valid_Add_iff : forall lb x v m m',
  ...
  Key.valid lb x /\ valid lb m <-> valid lb m'.
Proof.
  intros. unfold Add in H; rewrite H.
  apply valid_add_iff.
Qed.
...
Lemma shift_Add_iff : forall lb k key v (m m' : t),
  ...
  Add (Key.shift lb k key) v (shift lb k m) (shift lb k m').
Proof.
  intros; split; intros; unfold Add in *.
  - rewrite H. now rewrite shift_add_spec.
  - rewrite <- shift_add_spec in H.
    rewrite <- shift_eq_iff in H.
    assumption.
Qed.
...
Proof.
  ...
  - unfold Add in H0; rewrite H0.
    rewrite shift_add_spec with (lb := lb) (k := k + k'); auto.
    rewrite shift_add_spec with (lb := lb) (k := k'); auto.
    rewrite shift_add_spec with (lb := lb) (k := k).
...
Proof.
  ...
  - unfold Add in H0; rewrite H0.
    rewrite shift_add_spec with (lb := lb) (k := k'); auto.
    repeat rewrite shift_add_spec with (lb := lb) (k := k); auto.
    rewrite shift_add_spec with (lb := lb) (k := k'); auto.
...
Proof.
  ...
  - unfold Add in H0; rewrite H0. destruct (Key.eq_dec x x0).
    -- apply Key.eq_leibniz in e0; subst. rewrite remove_add_1.
       rewrite shift_add_spec; auto. now rewrite remove_add_1.
...
Proof.
  ...
  - eapply valid_Add_spec in Hvt; eauto; destruct Hvt as [Hv Hvt'].
    ...
    rewrite <- valid_Add_spec; eauto. now apply shift_notin_iff .
Qed.
