...
Proof. repeat red; intros; apply Key.eq_leibniz in H; subst. now rewrite H1. Qed.
...
Proof.
  ...
  - apply (valid_Add_iff _ x e m1 m2) in H1 as [Hvr [Hvpt Hv]]; auto.
...
Lemma shift_Add_spec_2 : forall lb k key v (m m' : t),
  ~ M.In key m -> Add key v m m' ->
  Add (Key.shift lb k key) (Data.shift lb k v) (shift lb k m) (shift lb k m').
Proof.
  intros. apply shift_Add_spec_1 with (lb := lb) (k := k) in H0; auto.
Qed.
...
Proof.
  ...
  - eapply shift_Add_spec_1 in H0; eauto. rewrite H0 in H1.
...
Lemma shift_Add_iff : forall lb k key v (m m' : t),
  ...
  Add (Key.shift lb k key) (Data.shift lb k v) (shift lb k m) (shift lb k m').
Proof.
  split; unfold Add; intros.
  - rewrite H. apply shift_add_spec.
  - rewrite <- shift_add_spec in H.
    now rewrite <- shift_eq_iff in H.
Qed.
...
Lemma shift_Add_spec : forall lb k key v (m m' : t),
  ...
  eq (shift lb k m') (M.add (Key.shift lb k key) (Data.shift lb k v) (shift lb k m)).
Proof.
  intros lb k key v m m' HAdd; unfold Add in *.
  rewrite HAdd. apply shift_add_spec. 
Qed.
...
Proof.
  ...
  - eapply valid_Add_iff in Hvt; eauto; destruct Hvt as [Hv [Hv' Hvt']].
    ...
    rewrite <- valid_Add_iff; eauto. now apply shift_notin_iff.
Qed.
...
Proof.
  ...
  - apply shift_Empty_iff with (lb := k) (k := (k' - k)) in H.
    now apply valid_Empty_spec.
  - eapply valid_Add_iff in H6; eauto; destruct H6 as [Hvr [HvData Hvt]].
